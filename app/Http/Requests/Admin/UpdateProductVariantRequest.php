...
class UpdateProductVariantRequest extends FormRequest
{
    ...
    {
        $variantId = (int) $this->route('variant')?->id;
    ...
    public function after(): array
    {
        return [
            function (Validator $validator): void {
                ...
                $requiresMembership = $this->boolean('is_active')
                    || $colorId != $variant->color_id
                    || $sizeId != $variant->size_id;

                if ($requiresMembership && $colorId && ! $product->colors()->whereKey($colorId)->exists()) {
                    $validator->errors()->add('color_id', __('The selected color is not attached to this product.'));
                }

                if ($requiresMembership && $sizeId && ! $product->sizes()->whereKey($sizeId)->exists()) {
                ...
                if ($product->variants()->where('variant_key', $variantKey)->whereKeyNot($variant->id)->exists()) {
