...
class B2BPhaseFiveMySqlConcurrencyTest extends TestCase
{
    public function test_two_connection_inventory_reservation_and_state_races(): void
    {
        ...
        try {
            ...
            $this->assertSame(1, InventoryMovement::where('product_variant_id', $reservationVariant->id)->count());
            ...
            $this->assertSame(1, InventoryMovement::where('product_variant_id', $replayVariant->id)->count());
            ...
            DB::purge('mysql');
            config(['database.connections.mysql.database' => $originalMysqlDatabase]);
        }
    }
    ...
    private function variant(string $sku): ProductVariant
    {
        ...
        $variant = ProductVariant::create(['product_id' => $product->id, 'sku' => $sku, 'is_active' => true, 'price_adjustment' => '0.00']);
        InventoryItem::create(['product_variant_id' => $variant->id, 'stock_on_hand' => 1, 'reserved_quantity' => 0]);
